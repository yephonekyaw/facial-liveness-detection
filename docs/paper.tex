...
near-zero error within their own test set yet fail catastrophically
when transferred to a different attack family. We argue that the
dominant cost in building a generalising classifier is not architectural
but \emph{logistical}: each public benchmark ships in a different
format, with a different annotation schema, and a different notion of
``split''. We present a unified RGB-only preprocessing pipeline that
collapses two structurally different datasets --- Replay-Attack
(2D photo and video presentations) and 3DMAD (3D mask attacks) ---
into a single CSV manifest of $77{,}296$ identically-shaped
$256{\times}256$ face crops, and we train AttackNet v2.2, a compact
$8.4$M-parameter convolutional network, from scratch on the union.
Within-dataset test ACER is below $0.5\%$ on both benchmarks. Held-out
cross-dataset evaluation reveals a sharp \emph{asymmetry} ---
Replay $\rightarrow$ 3DMAD transfers at $2.8\%$ frame ACER whereas
3DMAD $\rightarrow$ Replay collapses to $38.8\%$ --- which we interpret
as evidence that single-dataset models learn dataset-specific shortcuts
rather than a general notion of liveness. Combined training closes most
of this gap while incurring negligible within-domain cost.
The pipeline is also extensible: the manifest schema and a
matching extractor stub already accept a third dataset (CSMAD,
silicone mask attacks), which we leave to future work as a zero-shot
...
\ref{tab:cross-hter}, Figure~\ref{fig:cross-bars}) tell the real
story. Replay $\rightarrow$ 3DMAD is workable: at the fixed threshold
...
\section{Future Work: Cross-Dataset Probe with CSMAD}
\label{sec:future}

The pipeline is forward-compatible with a third dataset. CSMAD
(Custom Silicone Mask Attack Database) ships HDF5 captures from an
Intel RealSense SR300 (RGB, NIR, depth) and a Seek thermal camera,
with $87$ bonafide videos plus $159$ attack videos in two pose
families (\texttt{WEAR} and \texttt{STAND}); we use only the
SR300 \texttt{color} channel. Each frame in the HDF5 is a separate
timestamped dataset of shape $(H,W,3)$ \texttt{uint8} under
\texttt{data/sr300/color}; the extractor lists keys, sorts them
chronologically, and steps through with the same \texttt{FRAME\_STRIDE}
as the other extractors. We use a deterministic subject-letter split
(A--I train, J--K devel, L--N test) and tag every row with
\texttt{dataset='csmad'}, \texttt{attack\_type='real'} or
\texttt{'mask\_silicone'}. The training code does not need to change.

We deliberately do \emph{not} include CSMAD in the training mix in
this paper. CSMAD will instead be used as a held-out, third-domain
zero-shot probe of the combined model. The hypothesis to test is the
natural extension of Section~\ref{sec:cross-dataset}: a model trained
on a wider mix of attack families (2D presentations + 3D printed
masks) should transfer further into a fourth attack family (silicone
masks) than either single-dataset baseline. The relevant comparisons
are Replay $\rightarrow$ CSMAD, 3DMAD $\rightarrow$ CSMAD, and Combined
$\rightarrow$ CSMAD, all with devel-calibrated HTER. We leave the
running of those experiments to subsequent work, in which we will also
revisit whether the asymmetry observed here (Replay generalises, 3DMAD
does not) is preserved when the target distribution involves a third,
qualitatively different attack family.

\section{Conclusion}
\label{sec:conclusion}

We presented a unified RGB pipeline for face anti-spoofing that ingests
two structurally different public benchmarks --- Replay-Attack and
3DMAD --- into a single CSV manifest of $77{,}296$ uniformly cropped
face images, and we trained a compact $8{.}4$M-parameter CNN
(AttackNet v2.2) from scratch on their union. Within-dataset test
ACER is below $0{.}5\%$ on both benchmarks. Held-out cross-dataset
evaluation reveals a sharp asymmetry: Replay $\rightarrow$ 3DMAD
transfers at $2{.}80\%$ frame ACER, while 3DMAD $\rightarrow$ Replay
collapses to $38{.}75\%$. We interpret this as direct empirical
evidence that single-dataset PAD models learn dataset-specific
shortcuts rather than a general notion of liveness, and that combined
training closes most of the gap at negligible within-domain cost. The
manifest schema and matching extractor stub make adding a third
dataset (CSMAD, silicone masks) a configuration change rather than a
refactor; that experiment is the natural next step.
