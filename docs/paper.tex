...
Within-dataset test ACER is below $2\%$ on both benchmarks
(Replay $1{.}9\%$, 3DMAD $0\%$).
...
A held-out probe on the CSMAD corpus (silicone mask attacks, Intel
RealSense SR300) shows that cross-sensor transfer remains the dominant
failure mode even in the absence of attack-family novelty: the combined
model, trained without any CSMAD data, reaches a devel-calibrated HTER
of $0.46$ at ROC-AUC $0.63$ on the CSMAD test split, while the
replay-only model performs comparably (HTER $0.48$, AUC $0.63$).
Strikingly, the 3DMAD-only model transfers to CSMAD substantially
better than either (HTER $0.23$, AUC $0.85$), evidence that structural
overlap between rigid mask attacks survives the SR300 domain shift in
a way that screen- and print-attack features do not. Combined training
does not automatically inherit the better of the two transfers; for
silicone-mask threats specifically, the 3DMAD-conditioned prior is the
more useful one.
\end{abstract}
...
necessary, not optional; by-dataset, 3DMAD, Replay, and CSMAD each
occupy visibly different regions, which is the empirical motivation
for combined training across Replay and 3DMAD and the warning that
CSMAD --- whose cluster sits well outside both --- is unlikely to be
covered by it.
...
and t-SNE (bottom), coloured by label (left) and by source dataset
(right). Bonafide vs.\ attack overlap heavily; 3DMAD, Replay, and
CSMAD are each visibly separable as datasets. The relevant texture
cues are not represented in ImageNet features --- training from
scratch is therefore the right call.}
...
\textbf{Combined training is the minimum viable strategy for the
in-distribution datasets.} The combined model achieves $0{.}22\%$ CV
ACER (Table~\ref{tab:cv}) and matches the 3DMAD-only model exactly on
3DMAD-test ($0\%$ frame ACER) while running about $0{.}8\%$ behind the
Replay-only model on Replay-test ($1{.}85\%$ vs.\ $1{.}09\%$ frame
ACER). The within-domain cost of carrying a second dataset is small,
and the cross-dataset benefit is the entire Replay/3DMAD asymmetry
documented above. CSMAD is the case where this story does not hold:
combined training does not help on a third domain its data does not
...
\textbf{Implications.}
(i) The combined model does not automatically inherit the better of
the two single-dataset transfers; on CSMAD it inherits the worse one.
This pushes back on the naive interpretation that ``more diverse
training data is always better'' --- when one source is structurally
useful for a new target and the other is structurally orthogonal,
mixing them dilutes the useful prior.
(ii) The failure mode for an RGB-only PAD model on a new capture
device is not necessarily ranking inversion. Across the three models
all CSMAD AUCs are $> 0{.}5$, ranging from $0{.}63$ to $0{.}85$;
sensor shift hurts threshold calibration far more than it hurts
ranking.
(iii) Reporting only the combined model's CSMAD performance would
have hidden the most useful finding here, which is the asymmetric
transferability of mask-attack features. The per-source breakdown
matters for any practitioner trying to decide what data to add when
extending a PAD system to a new hardware target.
...
Within-dataset test ACER is below $2\%$ on both benchmarks
($1{.}9\%$ on Replay, $0\%$ on 3DMAD for the combined model).
...
finding: the combined model achieves HTER $0{.}46$ and ROC-AUC $0{.}63$
on CSMAD, the replay-only model performs comparably (HTER $0{.}48$,
AUC $0{.}63$), and the 3DMAD-only model transfers far better
(HTER $0{.}23$, AUC $0{.}85$). All three rank correctly --- there is
no ranking inversion --- but the combined model fails to inherit the
3DMAD model's CSMAD-friendly prior; its CSMAD score distribution is
dominated by the replay-attack feature direction. The practical
implication: when extending a PAD system to a new sensor and a new
mask family, the choice of training source matters more than its
size, and structural similarity between training and target attack
types is the variable that survives a sensor change.
