...
\begin{tabular}{c}
\toprule
Raw input: \texttt{.mov} containers (Replay-Attack) or \texttt{.h5} stacks (3DMAD, CSMAD) \\
$\Downarrow$ \\
Dataset-specific extractor; every $5$th frame sampled (\texttt{FRAME\_STRIDE} $= 5$) \\
$\Downarrow$ \\
RetinaFace face detector (\texttt{insightface buffalo\_sc}, ONNX, GPU) \\
$\Downarrow$ \\
Highest-confidence box $\xrightarrow{\;\times 1.3\;}$ square-pad to longer side $\xrightarrow{\;\text{Lanczos}\;}$ $256{\times}256$ \\
$\Downarrow$ \\
JPEG ($q{=}95$) written to \texttt{data/processed/} \quad$+$\quad row appended to \texttt{manifest.csv} \\
\bottomrule
\end{tabular}
\caption{Unified preprocessing pipeline. A single shared detector and a
fixed crop geometry are applied identically across all three datasets,
ensuring that the training loop receives a homogeneous input distribution
